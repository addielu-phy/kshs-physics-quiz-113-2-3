...
function isMulti(q) { return q.type === "multi" || Array.isArray(q.a); }
function answerKey(q) { return isMulti(q) ? q.a.slice().sort((a, b) => a - b) : [q.a]; }
function cleanMultiAnswer(ans) {
  return Array.isArray(ans) ? ans.slice().map(Number).filter(Number.isInteger).sort((a, b) => a - b) : [];
}
function hasAnswer(q, ans) { return isMulti(q) ? cleanMultiAnswer(ans).length > 0 : ans !== undefined; }
function sameList(a, b) { return a.length === b.length && a.every((v, i) => v === b[i]); }
function isCorrect(q, ans) {
  return isMulti(q) ? sameList(cleanMultiAnswer(ans), answerKey(q)) : ans === q.a;
}
function questionMax(q) { return q.points || (isMulti(q) ? 5 : QUIZ_META.perScore); }
function questionScore(q, ans) {
  if (!hasAnswer(q, ans)) return 0;
  if (!isMulti(q)) return ans === q.a ? questionMax(q) : 0;
  const picked = new Set(cleanMultiAnswer(ans));
  const correct = new Set(answerKey(q));
  const optionCount = q.o.length;
  let errors = 0;
  for (let i = 0; i < optionCount; i++) {
    if (picked.has(i) !== correct.has(i)) errors++;
  }
  if (errors === 0) return questionMax(q);
  if (errors === 1) return 3;
  if (errors === 2) return 1;
  return 0;
}
function attemptMaxScore(ids) { return ids.reduce((sum, id) => sum + questionMax(qById(id)), 0); }
function answerLabel(q, ans) {
  if (!hasAnswer(q, ans)) return "未作答";
  return (isMulti(q) ? cleanMultiAnswer(ans) : [ans]).map(i => LETTERS[i]).join("");
}
function correctLabel(q) { return answerKey(q).map(i => LETTERS[i]).join(""); }
function answeredCount(ids, answers) { return ids.filter(id => hasAnswer(qById(id), answers[id])).length; }
...
function viewLogin() {
  ...
  app.innerHTML = `
    <div class="brand"><div class="logo">物</div>
      <div><h1>${QUIZ_META.subject}自學評量</h1>
    ...
    <div class="foot">${QUIZ_META.note}<br>成績資料儲存在本機瀏覽器，清除瀏覽資料會一併刪除。</div>
  `;
    ...
    history = atts.map((a, i) => {
      ...
      return `<div class="att">
        ...
        <div class="grow">
          ...
          <div class="muted small">得分 ${a.score}/${a.maxScore || a.total * QUIZ_META.perScore}・答對 ${a.correct}/${a.total} 題・用時 ${fmtDur(a.durationSec)}・${fmtDate(a.date)}</div>
        </div>
...
function scorePct(a) { return Math.round(a.score / (a.maxScore || a.total * QUIZ_META.perScore) * 100); }
function trendBars(atts) {
  const max = 100;
  return `<div class="row" style="align-items:flex-end;gap:8px;height:84px;margin:4px 0 14px">
    ${atts.map(a => {
      const h = Math.max(6, Math.round(scorePct(a) / max * 72));
...
function renderQuestion() {
  ...
  const answered = answeredCount(s.ids, s.answers);
  const current = s.answers[id];

  const opts = q.o.map((txt, i) => {
    const sel = isMulti(q) ? cleanMultiAnswer(current).includes(i) : current === i;
    return `<button class="opt${sel ? " sel" : ""}" onclick="pick(${i})">
  ...
  const nav = s.ids.map((qid, i) => {
    let c = "";
    if (i === s.idx) c += " cur";
    if (hasAnswer(qById(qid), s.answers[qid])) c += " done";
  ...
  app.innerHTML = `
    ...
    <div class="card">
      <div class="qhead">
        ...
        ${isMulti(q) ? '<span class="chip">多選題，可複選</span>' : ""}
      </div>
      ${q.oi ? figuresHTML(q) : ""}
...
window.pick = function (i) {
  ...
  const q = qById(id);
  if (isMulti(q)) {
    const picked = new Set(cleanMultiAnswer(session.answers[id]));
    if (picked.has(i)) picked.delete(i); else picked.add(i);
    session.answers[id] = Array.from(picked).sort((a, b) => a - b);
    renderQuestion();
    return;
  }
  session.answers[id] = i;
  // 單選題自動前往下一題（最後一題則停留）；多選題由學生選好後手動前進。
...
function renderResult(name, index) {
  ...
  const review = showIds.length ? showIds.map(id => {
    ...
    const picked = new Set(isMulti(q) ? cleanMultiAnswer(my) : (my === undefined ? [] : [my]));
    const correct = new Set(answerKey(q));
    const earned = questionScore(q, my);
    const opts = q.o.map((txt, i) => {
      let cls = "opt";
      if (correct.has(i)) cls += " correct";
      else if (picked.has(i)) cls += " wrong";
      const mk = correct.has(i) ? "正解" : (picked.has(i) ? "你選的" : "");
    ...
    return `<div class="review-item ${ok ? "ok" : "ng"}">
      <div class="qhead">
        ...
        <span class="chip unit">${q.u}</span>
        <span class="chip">${earned}/${questionMax(q)} 分</span>
        ${!hasAnswer(q, my) ? '<span class="chip">未作答</span>' : ""}
      </div>
      <div class="stem">${q.s}</div>
      ${figuresHTML(q)}
      <div class="opts">${opts}</div>
  ...
  app.innerHTML = `
    ...
    <div class="card center">
      ...
      <div class="score-big">${att.score}<span style="font-size:1.2rem;color:var(--muted)"> / ${att.maxScore || att.total * QUIZ_META.perScore}</span></div>
...
function renderPractice() {
  ...
  const answeredTotal = answeredCount(s.ids, s.answers);
  ...
  const opts = q.o.map((txt, i) => {
    let cls = "opt";
    if (!revealed && picked.has(i)) cls += " sel";
    if (revealed) { if (correct.has(i)) cls += " correct"; else if (picked.has(i)) cls += " wrong"; }
    const mk = revealed ? (correct.has(i) ? "正解" : (picked.has(i) ? "你選的" : "")) : "";
  ...
  const feedback = revealed ? `
    <div class="card tight" style="border-color:${ok ? "var(--good)" : "var(--bad)"}">
      <div class="qhead"><span class="chip ${ok ? "good" : "bad"}">${ok ? "✓ 答對了！" : "✗ 答錯了，正解是 " + correctLabel(q)}</span>
      <span class="chip">${questionScore(q, my)}/${questionMax(q)} 分</span></div>
      <div class="expl"><b>💡 詳解：</b>${q.e}</div>
    </div>` : "";
  ...
  const nextBtn = revealed
    ...
    : (isMulti(q)
      ? `<button class="btn primary block" onclick="revealPractice()" ${hasAnswer(q, my) ? "" : "disabled"}>確認答案</button>
         <p class="muted small center" style="margin:6px 0">可複選，選好所有答案後再確認</p>`
  ...
  app.innerHTML = `
    ...
    <div class="card tight">
      <div class="spread" style="margin-bottom:8px">
        <span class="muted small">第 ${s.idx + 1} / ${total} 題</span>
        <span class="muted small">答對 ${correctCount} / 已答 ${answeredTotal}</span>
      </div>
      <div class="pbar"><span style="width:${(s.idx + (revealed ? 1 : 0)) / total * 100}%"></span></div>
    </div>
    <div class="card">
      <div class="qhead"><span class="qno">${q.id}.</span><span class="chip unit">${q.u}</span>${isMulti(q) ? '<span class="chip">多選題，可複選</span>' : ""}</div>
      ${q.oi ? figuresHTML(q) : ""}
      <div class="stem">${q.s}</div>
      ${q.oi ? "" : figuresHTML(q)}
      <div class="opts">${opts}</div>
...
window.revealPractice = function () { if (!session.revealed) { session.revealed = true; renderPractice(); } };
...
window.finishPractice = function () {
  ...
  const attempt = {
    ...
    correct, total: s.ids.length, score, maxScore, wrongIds,
    durationSec: Math.max(1, Math.round((Date.now() - s.start) / 1000))
  };
...
function renderPracticeSummary(name, att) {
  ...
  const wrong = att.wrongIds.map(id => {
    const q = qById(id);
    const correctOpts = answerKey(q).map(i =>
      `<div class="opt correct"><span class="k">${LETTERS[i]}</span><span>${q.oi ? "選項 " + LETTERS[i] : q.o[i]}</span><span class="mk">正解</span></div>`
    ).join("");
    return `<div class="review-item ng">
      ...
      <div class="opts">${correctOpts}</div>
  ...
  app.innerHTML = `
    ...
    <div class="card center">
      ...
      <div style="margin:4px 0 2px">得分 ${att.score}/${att.maxScore || attemptMaxScore(att.ids)}・正確率 ${p}%・用時 ${fmtDur(att.durationSec)}</div>
      <div style="margin-top:10px"><span id="cloudStatus" class="chip" style="display:none"></span></div>
    </div>
