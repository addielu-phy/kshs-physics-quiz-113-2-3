  ...
  const list = s.all.slice().sort((a, b) => (tsToDate(b) || 0) - (tsToDate(a) || 0)).map(a => {
    ...
    return `<div class="att">
      ...
      <div class="grow">
        ...
        得分 ${a.score}/${a.maxScore || QUIZ_META.total}・答對 ${a.correct}/${a.total}・用時 ${fmtDur(a.durationSec || 0)}
...
window.exportCSV = function () {
  ...
  ROWS.slice().sort((a, b) => (tsToDate(a) || 0) - (tsToDate(b) || 0)).forEach(r => {
    const row = [
      ...
      r.score, r.maxScore || QUIZ_META.total, r.correct, r.total, (r.wrongIds || []).length,
